  ...
  bool checkNormalValue(int gender, SelectedScaleType type, double val) {
    switch (type) {
      case SelectedScaleType.bmh:
        return (val >= ScaleConstants.bMHRanges[0]) ? true : false;

      case SelectedScaleType.bodyFat:
        {
          final ranges = ScaleConstants.bodyFatRanges(gender);
          return (val >= ranges[0] && val < ranges[1]) ? true : false;
        }

      case SelectedScaleType.bmi:
        return (val >= ScaleConstants.bMIRanges[0] &&
                val < ScaleConstants.bMIRanges[2])
            ? true
            : false;

      case SelectedScaleType.muscle:
        {
          final ranges = ScaleConstants.muscleRanges(gender);
          return (val >= ranges[0] && val < ranges[1]) ? true : false;
        }

      case SelectedScaleType.water:
        {
          final ranges = ScaleConstants.waterRanges(gender);
          return (val >= ranges[0] && val < ranges[1]) ? true : false;
        }

      case SelectedScaleType.visceralFat:
        return (val <= ScaleConstants.visceralFatRanges[0]) ? true : false;

      case SelectedScaleType.boneMass:
        return (val >= ScaleConstants.boneMassRanges[0] &&
                val < ScaleConstants.boneMassRanges[1])
            ? true
            : false;
