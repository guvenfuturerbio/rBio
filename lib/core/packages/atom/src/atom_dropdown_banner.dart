...
class AtomDropdownBanner extends StatefulWidget {
  /// Builder in which to construct the app content that you are wrapping
  final Widget child;
  final GlobalKey<NavigatorState> navigatorKey;

  AtomDropdownBanner({required this.child, required this.navigatorKey}) {
    _DartNotificationCenter.registerChannel(channel: _BANNERCHANNEL);
  }
  ...
  @override
  _AtomDropdownBannerState createState() => _AtomDropdownBannerState();
}

class _AtomDropdownBannerState extends State<AtomDropdownBanner> {
  ...
  void initState() {
    ...
    _DartNotificationCenter.subscribe(
      channel: _BANNERCHANNEL,
      observer: this,
  ...
  @override
  void dispose() {
    _DartNotificationCenter.unsubscribe(observer: this);
    banners = [];
    super.dispose();
  }
...
class _BannerInstanceState extends State<_BannerInstance> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback(
      (duration) => setState(
        () => this.bannerHeight = context.size!.height,
      ),
    );
  }
  ...
  build(BuildContext context) {
    final top =
        bannerHeight == null ? -120.0 : (isActive ? 0.0 : -(bannerHeight ?? 0));
...
class _DartNotificationCenter {
  ...
  static _DartNotificationCenter _sharedCenter = _DartNotificationCenter._();

  _DartNotificationCenter._();
