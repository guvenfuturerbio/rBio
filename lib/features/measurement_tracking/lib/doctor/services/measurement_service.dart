...
class MeasurementService {
  ...
  List<BgMeasurement> fetchBgMeasurements() {
    List<BgMeasurement> bgMeasurements = [];
    bgMeasurements = measurements
        .map((e) => BgMeasurement(
            date: e.date,
            result: e.result,
            tag: e.tag,
            color: fetchMeasurementColor(
              ...
              measurement: int.parse(e.result),
              targetMax: PatientNotifiers()?.patientDetail?.rangeMax ?? 0,
              targetMin: PatientNotifiers()?.patientDetail?.rangeMin ?? 0,
            )))
        .toList();
    print(bgMeasurements.toString());
    return bgMeasurements;
  }

  Future<List<BloodGlucose>> fetchBgMeasurements1(int id) async {
    return await PatientService().getPatientBloodGlucose(
        patientId: id,
        getMyPatientFilter: GetMyPatientFilter(end: null, start: null));
  }

  /*Future<List<BgMeasurement>> fetchBgMeasurements() async {
    List<BgMeasurement> bgMeasurements = new List<BgMeasurement>();
    List<GlucoseData> glucoseDatas = await GlucoseRepository().getAllGlucoseData();
    bgMeasurements = glucoseDatas.map((e) => BgMeasurement(
        result: e.level,
        date: DateTime.fromMicrosecondsSinceEpoch(e.time * 1000).toString(),
        color: fetchMeasurementColor(int.parse(e.level)),
        tag: e.tag,
        notes: e.note,
        imageURL: e.imageURL,
  ...
  /*void addDataToDataBase(){
    for(var data in measurements){
      //print(DateTime.parse(data.date).millisecond.toString());
      GlucoseRepository().addNewGlucoseData(GlucoseData(level: data.result, time: DateTime.parse(data.date).millisecondsSinceEpoch, tag: data.tag));
    }
  }*/
}
