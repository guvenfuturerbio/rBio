...
class PatientDetailPageViewModel with ChangeNotifier {
  ...
  Map<Color, String> _colorInfo = Map<Color, String>();

  Map<Color, String> get colorInfo {
    _colorInfo.putIfAbsent(
        R.color.veryLow, () => LocaleProvider.current.very_low);
    _colorInfo.putIfAbsent(R.color.low, () => LocaleProvider.current.low);
    _colorInfo.putIfAbsent(R.color.target, () => LocaleProvider.current.target);
    _colorInfo.putIfAbsent(R.color.high, () => LocaleProvider.current.high);
    _colorInfo.putIfAbsent(
        R.color.veryHigh, () => LocaleProvider.current.very_high);
  ...
  Map<String, bool> _filterState = Map<String, bool>();

  Map<String, bool> get filterState {
    _filterState.putIfAbsent(LocaleProvider.current.very_low, () => true);
    _filterState.putIfAbsent(LocaleProvider.current.low, () => true);
    _filterState.putIfAbsent(LocaleProvider.current.target, () => true);
    _filterState.putIfAbsent(LocaleProvider.current.high, () => true);
    _filterState.putIfAbsent(LocaleProvider.current.very_high, () => true);
    _filterState.putIfAbsent(LocaleProvider.current.hungry, () => true);
    _filterState.putIfAbsent(LocaleProvider.current.full, () => true);
    _filterState.putIfAbsent(LocaleProvider.current.other, () => true);
    return this._filterState;
  }

  Future<void> setFilterState(String key) async {
  ...
  updateBgMeasurement() async {
    ...
    this.bgMeasurements.removeWhere((element) =>
        (!isFilterSelected(LocaleProvider.current.very_high) &&
            element.resultColor == R.color.veryHigh) ||
        (!isFilterSelected(LocaleProvider.current.high) &&
            element.resultColor == R.color.high) ||
        (!isFilterSelected(LocaleProvider.current.target) &&
            element.resultColor == R.color.target) ||
        (!isFilterSelected(LocaleProvider.current.low) &&
            element.resultColor == R.color.low) ||
        (!isFilterSelected(LocaleProvider.current.very_low) &&
            element.resultColor == R.color.veryLow) ||
        (!isFilterSelected(LocaleProvider.current.hungry) &&
            element.tag == 1) ||
        (!isFilterSelected(LocaleProvider.current.full) && element.tag == 2) ||
        (!isFilterSelected(LocaleProvider.current.other) &&
            element.tag != 1 &&
            element.tag != 2));
    Provider.of<BgMeasurementsNotifierDoc>(context, listen: false)
