...
class DoctorChatController with ChangeNotifier {
  ...
  String username;
  UserNotifiers userProfilesNotifier = locator<UserNotifiers>();
  ...
  Future<void> sendNotification(Message message) async {
    ...
    String json =
        '{ "to" : "$token", "notification" :{ "title" : "${userProfilesNotifier.userName}", "body" : "${message.message}" },  "data" : { "type":"3","message" : "${message.message}", "title": "${userProfilesNotifier.userName} ","id": "${userProfilesNotifier.userId}" } }';
