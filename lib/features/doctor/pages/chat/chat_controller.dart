...
class DoctorChatController with ChangeNotifier {
  ...
  Future<void> sendNotification(Message message) async {
    ...
    // String json =
    //     '{ "to" : "$token", "notification" :{ "title" : "${userName}", "body" : "${message.message}" },  "data" : { "type":"3","message" : "${message.message}", "title": "${userProfilesNotifier.userName} ","id": "${userProfilesNotifier.userId}" } }';
