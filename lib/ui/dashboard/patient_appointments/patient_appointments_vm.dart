...
class PatientAppointmentsScreenVm extends ChangeNotifier {
  ...
  PatientAppointmentsScreenVm({BuildContext context, String jitsiRoomId}) {
  ...
  get startDate => _startDate != null
      ? DateTime(_startDate.year, _startDate.month, _startDate.day)
      : DateTime(
          DateTime.now().year, DateTime.now().month - 1, DateTime.now().day);

  setStartDate(DateTime d) async {
    this._startDate = d;
    fetchPatientAppointments();
    notifyListeners();
  }

  get endDate => _endDate != null
      ? DateTime(_endDate.year, _endDate.month, _endDate.day, 23, 59, 59)
      : DateTime(
          DateTime.now().add(Duration(days: 1)).year,
          DateTime.now().add(Duration(days: 1)).month + 1,
                ...
                actions: [
                  Row(
                    ...
                    children: [
                      ...
                      //
                      button(
                        text: LocaleProvider.of(mContext).confirm,
                        width: 100,
                        onPressed: () {
                          Navigator.pop(mContext);
                          Atom.to(
